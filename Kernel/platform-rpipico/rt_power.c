...
// user choice (?, ?)
// power save (min volt, min freq)
// balanced (default volt, max freq)
// max perf (max volt, max freq)
// ram mode: run stable from ram and have GPIO only
static power_level_t speed[5] = {
...
// test status
static uint8_t test_status = POWER_TEST_STATUS_UNKNOWN;
// burn test min index
static uint8_t test_index_min = 0;
// burn test max index
static uint8_t test_index_max = 0;
// burn test min/max select
static uint8_t test_index_select = 0;
...
static void power_test_reset(void) {
    ...
    uint8_t test_index_min = 0;
    uint8_t test_index_max = 0;
    uint8_t test_index_select = 0;
...
static void power_test_set_level(bool min, uint8_t index) {
