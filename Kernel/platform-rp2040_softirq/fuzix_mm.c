...
// placeholder to prevent RT layer from allocating heap memory
uint8_t *progbase;

// allocation table
pagemap_entry_t *allocation_map;
...
void pagemap_init(void)
{
    ...
    //INFO("%p HEAP_BASE", HEAP_BASE);
    //INFO("%p PROGBASE", PROGBASE);
    //INFO("%p = %p + %lu (PROGLOAD = udata + UDATA_SIZE)", PROGBASE+UDATA_SIZE, &udata, (uint32_t)UDATA_SIZE);
    //INFO("%p PROGLOAD", PROGLOAD);
    //INFO("%p = %p + %lu (PROGTOP = PROGBASE + PROCESS_MAX_MEM)", PROGTOP, PROGBASE, PROCESS_MAX_MEM);
    //INFO("%lu = %lu - %lu (PROGSIZE = PROCESS_MAX_MEM-UDATA_SIZE)", PROGSIZE, PROCESS_MAX_MEM, UDATA_SIZE);
    ...
    //INFO("%lu = Pico mem size", SRAM_END-SRAM_BASE);
    //INFO("%lu = Heap size", HEAP_END-HEAP_BASE);
    //INFO("%lu = FUZIX_MEM_SIZE", FUZIX_MEM_SIZE);
    //INFO("%lu = fuzix_process_memory", fuzix_process_memory);

	// The total amount of memory available in the system, including memory used for the kernel and other resources
	ramsize = fuzix_process_memory / 1024;

    // set process mem to busy
    //progbase = malloc(fuzix_process_memory);

	// sanity check
    if ((uaddr_t)PROGBASE!=(uaddr_t)progbase) {
        INFO("--- Process memory mismatch ---");
        INFO("%p HEAP_BASE", HEAP_BASE);
        INFO("%p PROGBASE", PROGBASE);
        INFO("%p progbase[%d]", progbase, fuzix_process_memory);
        INFO("offset %ld", (int)((int)progbase)-(int)PROGBASE);
        panic("PROGBASE != progbase");
    }

    allocation_map = malloc(sizeof(pagemap_entry_t)*NUM_ALLOCATION_BLOCKS);

	// the number of Kbytes of memory available for process allocation
	procmem = (FUZIX_MEM_SIZE-sizeof(pagemap_entry_t)*NUM_ALLOCATION_BLOCKS) / 1024;
    //--------------- end ------------------
