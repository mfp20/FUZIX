...
Everything runs on Core0, leaving Core1 free for real time applications.
...
Once the board has rebooted it will appear as multiple USB serial devices which you can connect to.
Alternatively, connect a terminal to Pico's UART0 default pins (115200 baud, 8N1, no control flow).

# Installation Details

Out of the box Fuzix understands DOS partition tables so disks can be partitioned but default disks aren't.
Disks are not hot swappable as the USB and SD card are only probed at boot time.

By default 3 extra disk images are generated:
...
Disks mapping depends on the disks availability found on boot; USB is tested first, then SD card, FLASH is last:

  - /dev/hda is the root disk. If USB is available at boot, it will be the first disk served by `pyco.py` utility on host.
    If USB isn't available and SD card is, it will be the first partition on SD card. Otherwise will be the NAND flash.
  
  - /dev/hdb is the swap disk. If USB is available at boot, it will be the second disk served by `pyco.py` utility on host.
    If USB isn't available and SD card is, it will be the second partition on SD card. Otherwise Fuzix will run in swapless mode
    and system's capabilities will be very limited.

  - /dev/hdc is the scratch disk. If USB is available at boot, it will be the third disk served by `pyco.py` utility on host.
    If USB isn't available and SD card is, it will be the third partition on SD card. Otherwise is unused.

  - /dev/hdd is an extra disk. In the case both USB and SD card are available it will be the third SD card partition.
    If one of the two is available it will be the flash root partition. Otherwise is unused.

  - /dev/hde is an extra disk. In the case both USB and SD card are available it will be the flash root partition. Otherwise is unused.

## USB

By connecting the Pico up via USB to a PC it presents itself as a few standard
...
**Note1:** when using the USB console, you're unlikely to be able to connect
...
**Note2:** on Windows the COM number varies. You must figure out which COM port corresponds
to tty1, tty2 and tty3.
...
The first thing you probably want to do is `setdate`.
Date and time are kept thanks to Pico's RTC in the case of always on, battery supported setup.
If USB is available there's no need to setdate as date is sync'ed to host at boot.
If no battery and USB are available, the system reads `/etc/datetime` and use that date and time
to set the RTC at boot and you should manually `setdate` in order to avoid
painful time screws.

The second thing you might need is `stty erase '^?'` to make the DELETE
